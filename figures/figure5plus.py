...
MTE = []; blabels=[]
cap=16;
deltalist=np.logspace(-1,1,100)

exact_solution = ss.mteSum1D(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], deltalist), deltalist, tech="sum1d")
...
MTE.append(ss.mteFP_full22(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], variability[var]), deltalist)); blabels.append("FP full")
...
#FP GAUSSIAN
MTE.append(ss.mteFP_gaussian(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], variability[var]), deltalist)); blabels.append("FP Gaussian")
#WKB REALSPACE
MTE.append(1.*ss.mteDist(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], variability[var]), deltalist, "WKB_RS")); blabels.append("WKB")
#QSD ALGORITHM
...
#EXACT SOLUTION
dummyvar=ss.mteSum1D(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], deltalist), deltalist, tech="sum1d");
...
print(len(MTE))
#print(cap)
print(len(ss.mteFP_Laplace_arrayed2(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], variability[var]), deltalist)))
print(ss.mteFP_Laplace_arrayed2(cap, stochasticity[sto], cap, ss.maxPop(cap, stochasticity[sto], variability[var]), deltalist)[-1])
...
ax.set_xlabel(r"basal death rate $\delta$",fontsize=fnt)
...
#plt.savefig('C:\\Users\\lenov\\Pictures\\'+figname+'-delta-K16.pdf',transparent=True)#, bbox_inches='tight')
plt.show(); 
